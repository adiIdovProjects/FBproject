...
class CampaignRepository(BaseRepository):
    ...
    def get_campaign_comparison(
        ...
        Returns:
            Dict mapping campaign_id to metrics dict
        """
        # Build account filter
        account_filter = ""
        param_account_ids = {}
        ...
        campaign_placeholders = ', '.join([f":campaign_id_{i}" for i in range(len(campaign_ids))])
        ...
        query = text(f"""
            ...
            WHERE d.date >= :start_date
                AND d.date <= :end_date
                AND f.campaign_id IN ({campaign_placeholders})
                {account_filter}
        ...
        comparison_data = {}
        for row in results:
            comparison_data[int(row.campaign_id)] = {
                'spend': float(row.spend or 0),
                'impressions': int(row.impressions or 0),
                'clicks': int(row.clicks or 0),
                'conversions': int(row.conversions or 0),
    ...
    def get_campaigns_for_manage(
        ...
        Returns campaigns with calculated metrics (CTR, CPC, CPA, Conv Rate).
        """
        # Build account filter
        account_filter = ""
        param_account_ids = {}
