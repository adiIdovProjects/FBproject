...
/**
 * CampaignComparisonModal Component
 * Displays side-by-side comparison of 2-5 campaigns with winner highlights
 */

import React, { useState, useEffect } from 'react';
import { X, Loader2, Crown } from 'lucide-react';
import { fetchCampaignComparison } from '../../services/campaigns.service';
import { DateRange, CampaignRow, CampaignComparisonResponse } from '../../types/campaigns.types';

interface CampaignComparisonModalProps {
  campaignIds: number[];
  campaigns: CampaignRow[];  // To get names/titles
  dateRange: DateRange;
...
export const CampaignComparisonModal: React.FC<CampaignComparisonModalProps> = ({
  campaignIds,
  campaigns,
  dateRange,
  accountId,
  ...
  const [comparisonData, setComparisonData] = useState<CampaignComparisonResponse | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (isOpen && campaignIds.length >= 2) {
      loadComparisonData();
    }
  }, [isOpen, campaignIds, dateRange, accountId]);

  const loadComparisonData = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await fetchCampaignComparison(campaignIds, dateRange, accountId);
      setComparisonData(data);
    } catch (err: any) {
      console.error('[CampaignComparisonModal] Error loading comparison:', err);
  ...
  const getCampaignName = (campaignId: number): string => {
    ...
    return campaign?.campaign_name || `Campaign ${campaignId}`;
  };
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-border-subtle">
                    ...
                    {comparisonData.campaign_ids.map((campaignId) => (
                      <th
                        key={campaignId}
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {comparisonData.comparisons.map((comparison) => (
                    <tr key={comparison.metric_name} className="hover:bg-white/[0.02]">
                      ...
                      {comparisonData.campaign_ids.map((campaignId) => {
                        const value = comparison.values[campaignId];
                        const isWinner = comparison.winner_id === campaignId;
