...
/**
 * TargetingComparisonModal Component
 * Displays side-by-side comparison of 2-5 ad sets with winner highlights
 */

import React, { useState, useEffect } from 'react';
import { X, Loader2, Crown } from 'lucide-react';
import { fetchTargetingComparison } from '../../services/campaigns.service';
import { DateRange } from '../../types/campaigns.types';
import { TargetingRow, TargetingComparisonResponse } from '../../types/targeting.types';

interface TargetingComparisonModalProps {
  adsetIds: number[];
  targetingData: TargetingRow[];  // To get names
  dateRange: DateRange;
...
export const TargetingComparisonModal: React.FC<TargetingComparisonModalProps> = ({
  adsetIds,
  targetingData,
  dateRange,
  accountId,
  ...
  const [comparisonData, setComparisonData] = useState<TargetingComparisonResponse | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (isOpen && adsetIds.length >= 2) {
      loadComparisonData();
    }
  }, [isOpen, adsetIds, dateRange, accountId]);

  const loadComparisonData = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await fetchTargetingComparison(adsetIds, dateRange, accountId);
      setComparisonData(data);
    } catch (err: any) {
      console.error('[TargetingComparisonModal] Error loading comparison:', err);
  ...
  const getAdsetName = (adsetId: number): string => {
    const adset = targetingData.find(a => a.adset_id === adsetId);
    return adset?.adset_name || `Ad Set ${adsetId}`;
  };
  ...
  const formatMetricValue = (metricName: string, value: number): string => {
    ...
    } else if (metricName === 'spend' || metricName === 'cpc' || metricName === 'cpa') {
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-border-subtle">
                    ...
                    {comparisonData.adset_ids.map((adsetId) => (
                      <th
                        key={adsetId}
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {comparisonData.comparisons.map((comparison) => (
                    <tr key={comparison.metric_name} className="hover:bg-white/[0.02]">
                      ...
                      {comparisonData.adset_ids.map((adsetId) => {
                        const value = comparison.values[adsetId];
                        const isWinner = comparison.winner_id === adsetId;
...
export default TargetingComparisonModal;
